/*
 * Copyright (C) 2021 Cervon Wong and Lee I-Shiang
 */

import 'dart:ui';

class ColorConstants {
  ColorConstants._(); // Prevent instantiation with private constructor.

  static const blackPrimary = Color(0xFF1C1917);
  static const blackSecondary = Color(0xA61C1917); // 65%.
  static const blackTertiary = Color(0x661C1917); // 40%.

  static const whitePrimary = Color(0xFFFFFFFF);
  static const whiteSecondary = Color(0xB3FFFFFF); // 70%.
  static const whiteOverlay = Color(0x33FFFFFF); // 20%.

  static const greenPrimary = Color(0xFF329561);
  static const greenLightOverlay = Color(0x0D329561); // 5%.
  static const greenOverlay = Color(0x1A329561); // 10%.
  static const greenLightBorder = Color(0x33329561); // 20%.
  static const greenBorder = Color(0x4D329561); // 30%.
  static const greenDisabled = Color(0x66329561); // 40%.
  static const greenInactive = Color(0x80329561); // 50%.
  static const greenDark = Color(0xFF276640);
  static const greenOverlayOnWhite = Color(0xFFEBF5F0);

  static const ivoryPrimary = Color(0xFFFFFCEB);
  static const ivoryOverlay = Color(0xE6FFFCEB); // 90%.
  static const ivory100 = Color(0xFFFCEFCB);
  static const ivory200 = Color(0xFFF3D9A5);
  static const ivoryScrim = Color(0xA6FCEFCB); // 65%.

  static const redPrimary = Color(0xFFE05252);
  static const redLightOverlay = Color(0x0DE05252); // 5%.
}
